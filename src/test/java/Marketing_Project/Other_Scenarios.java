...
public class Other_Scenarios extends ExtentReport {
	...
	public void Invoice_Setup(String userId,String password,String url) throws Exception{
extentTest = extent.startTest("Invoice_Setup");
		...
		//Select Primary invoice template(Front)
		Select PITF = new Select(action_obj.PIT_Front());
		PITF.selectByVisibleText("Full Bill Version 1.0");

		//Select Primary invoice template(Back)
		Select PITB = new Select(action_obj.PIT_Back());
		PITB.selectByVisibleText("Full Bill Version 1.0");
	
	
		//Select Secondary invoice template(Front)
		Select SITF = new Select(action_obj.SIT_Front());
		SITF.selectByVisibleText("Full Bill Version 1.0");
		
		//Select Primary invoice template(Back)
		Select SITB = new Select(action_obj.SIT_Back());
		SITB.selectByVisibleText("Full Bill Version 1.0");
...
//Thread.sleep(5600);
	...
	@Test(priority=1)
	public void Email_Template_setup() throws Exception{
		extentTest = extent.startTest("Email_Template_setup");
		...
		action_obj.Templates().click();
		Thread.sleep(3000);

		driver.navigate().refresh();

		//Select Invoice from Message Type dropdown 
		Select ms = new Select(action_obj.Message_Type());
		ms.selectByValue("INV");
		Thread.sleep(5000);
		
		//Enter Email from
		action_obj.Email_From().clear();
		Thread.sleep(1000);
		action_obj.Email_From().sendKeys("test@yopmail.com");
		
		//Enter Email Bcc
		action_obj.Email_Bcc().clear();
		Thread.sleep(1000);
		action_obj.Email_Bcc().sendKeys("test@yopmail.com");
		
		//Enter Email Subject
		action_obj.Email_Subject().clear();
		Thread.sleep(1000);		
		action_obj.Email_Subject().sendKeys("For Testing Purpose");
		
		//Enter Email Message		
		Thread.sleep(3000);
		//driver.switchTo().frame(driver.findElement(By.id("email_rem_message-wysiwyg-iframe")));
		//action_obj.Email_Messgae().clear();
		action_obj.Email_Messgae().sendKeys("Hi Everyone, This is for testing");
		
		executor.executeScript("window,scrollBy(0,3000)", "");
		Thread.sleep(3000);
		//Click on save button
		action_obj.Save_button().click();
		Thread.sleep(1000);
		
		//Assert success messsage
		Assertion_obj.Assertion_Success_Messages();  */
...
//		Thread.sleep(83488);
	...
	public void Add_Meter() throws Exception{
		...
		//Add Meter Number
		action_obj.Meter_Serial_Number().sendKeys("METER" + random2);
	...
	public void Add_Meter_Reads() throws Exception{
		...
		meternumber.selectByVisibleText("METER" + random2);
		Thread.sleep(6000);
		...
		meterno.selectByVisibleText("METER" + random2);
		Thread.sleep(3000);
		...
		meterno2.selectByVisibleText("METER" + random2);
		Thread.sleep(4000);
	...
	{   
		...
		action_obj.cycleName().sendKeys("TestBillRun"+ random2);
		Thread.sleep(3000);
...
Thread.sleep(2000);
		...
		billruncycle.selectByVisibleText("TestBillRun"+ random2);
	...
	public void Update_Statement_Dates() throws Exception{
		extentTest = extent.startTest("Update_Statement_Dates");
		...
		//Click on view details of the statement
		action_obj.View_Details().click();
		
		//Select any customer
		action_obj.Select_Checkbox().click();
		
		//Click on update statement dates
		action_obj.Update_Statement_Dates().click();
		
		//Click on new issue date
		action_obj.New_Issue_Date().click();
		...
		//Click on new due date
		action_obj.New_Due_Date().click();
		...
		//Assert Success Messgae
		Assertion_obj.Assertion_Success_Info_Message();	
		Thread.sleep(2000);
		
		//Assert Information popup
...
//Thread.sleep(10989);
	...
	public void Download_PDF() throws Exception{
    	extentTest = extent.startTest("Download_PDF");
		...
		action_obj.Download_Pdf_Icon().click();
		
		//Select 'Selected Customers' option
		action_obj.Selected_Customers_D().click();
		
		//Downlaod Button
		action_obj.Download_Button().click();	
    //Thread.sleep(6897);
}
    
    @Test(priority=10)
    public void Email() throws Exception{
    	extentTest = extent.startTest("Email");
    	action_obj = new Repo_testing(driver);
		Assertion_obj = new Assertion_Test(driver);
		executor = (JavascriptExecutor) driver;
		
		//click on Email button
		action_obj.Email_Bills_Icon().click();
		
		//Select all customer email
		action_obj.All_Customer_Email().click();
		
		//Click on Continue Email Button
		action_obj.Continue_Email().click();
    	
		//Validate Send Email Results popup 
	//	Assertion_obj.Assertion_Send_Email_Results_Popup(); 
    ...
    @Test(priority=11)
    public void Bill_Run_With_Uncommitted_Statement() throws Exception {
    	extentTest = extent.startTest("Bill_Run_With_Uncommitted_Statement");
    	action_obj = new Repo_testing(driver);
		Assertion_obj = new Assertion_Test(driver);
		executor = (JavascriptExecutor) driver;
		driver.navigate().refresh();
